...
build_abi() {
  ...
  # QuickJS-NG intentionally changes several predicate return types from int
  # to bool while retaining the same embedding API shape. Keep the authoritative
  ...
  sed \
    ...
    -e 's/c\.JS_IsFunction(state.context, module_dispatch) == 0/!c.JS_IsFunction(state.context, module_dispatch)/g' \
