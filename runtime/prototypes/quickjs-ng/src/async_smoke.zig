...
fn dumpException(ctx: *c.JSContext) void {
    ...
    if (message != null) {
        _ = c.fputs("QuickJS-NG Sting semantic smoke exception: ", c.stderr);
...
pub fn main() !void {
    ...
    c.JS_SetRuntimeInfo(runtime, "stingjs-quickjs-ng-async-smoke");
    installHostCall(ctx);

    try evaluate(ctx, bridge_bootstrap, "sting-quickjs-ng-async-bridge.js");
    ...
    std.debug.print("QuickJS-NG Solid 2 async semantics passed: promise/loading/pending/error/recovery stream action/optimistic\n", .{});

    try evaluate(ctx, bridge_bootstrap, "sting-quickjs-ng-conformance-bridge.js");
    ...
    std.debug.print("QuickJS-NG Solid 2 conformance suite passed\n", .{});
}
