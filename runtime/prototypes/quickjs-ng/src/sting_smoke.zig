...
fn dumpException(ctx: *c.JSContext) void {
    ...
    if (message != null) {
        _ = c.fputs("QuickJS-NG Sting smoke exception: ", c.stderr);
...
pub fn main() !void {
    ...
    c.JS_SetRuntimeInfo(runtime, "stingjs-quickjs-ng-sting-smoke");
    installHostCall(ctx);

    try evaluate(ctx, bridge_bootstrap, "sting-quickjs-ng-bridge.js");
    try evaluate(ctx, app_source, "sting-app.js");
    try runPendingJobs(runtime, ctx);

    if (smoke_state.button_id < 0) {
        std.debug.print("QuickJS-NG Sting smoke failed: native Add Button was not identified\n", .{});
        return SmokeError.InitialMountFailed;
    }
    if (!smoke_state.saw_initial_count or smoke_state.count_text_id < 0) {
        std.debug.print("QuickJS-NG Sting smoke failed: native Count: 0 text was not mounted\n", .{});
        return SmokeError.InitialMountFailed;
    }
